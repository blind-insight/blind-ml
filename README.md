...
| [`breast_cancer.ipynb`](breast_cancer.ipynb) | Breast cancer screening risk (HIPAA k=11 binning)  | Seven encrypted ML models (NB, GNB, BN, DT, RF, LR, Histogram) + Gail/BCSC benchmarks | 20K per batch — upload as many as you want         |
...
Trains seven encrypted ML models on BI aggregate counts with HIPAA k=11 suppression: Naive Bayes, Gaussian NB, Bayesian Network, Decision Tree, Random Forest, aggregate OLS/ridge Logistic Regression, and Histogram Classifier. Gail-model / BCSC clinical comparisons stay separate.
